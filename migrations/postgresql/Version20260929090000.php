...
final class Version20260929090000 extends AbstractMigration
{
    ...
    {
        return 'Curso académico de las entregas y completados anteriores a Version20260923130000 en actividades que empiezan antes del inicio del curso (PostgreSQL)';
    }
    ...
    public function up(Schema $schema): void
    {
        ...
        foreach (ActivityCycleCorrection::documents($this->connection) as $correction) {
            $this->addSql('UPDATE document SET activity_cycle_year = ? WHERE id = ?', [$correction['cycleYear'], $correction['id']], [ParameterType::INTEGER, ParameterType::STRING]);
        }
        foreach (ActivityCycleCorrection::completions($this->connection) as $correction) {
            $this->addSql('UPDATE activity_completion SET cycle_year = ? WHERE id = ?', [$correction['cycleYear'], $correction['id']], [ParameterType::INTEGER, ParameterType::STRING]);
        }
    }
